...
- *note* : care should be taken *not* to override the underlying ``mamba`` environment. For example, this should *not* attempt to do a ``mamba env update`` call as it will overwrite the base mamba environment and one ends up with a conda environment without the base mdakit dependencies.  
